...
if process_daymet:
    ...
    hs_attr_df = gpd.read_file(os.path.join(BASE_DIR, f'HYSETS_data/{hs_attrs_path}'), dtype_dict={'Official_ID': str})
...
basin_attrs = gpd.read_file(os.path.join(BASE_DIR, f'HYSETS_data/{out_path}'))
...
def main():
    ...
    hysets_fpath = os.path.join(BASE_DIR, 'HYSETS_data/HYSETS_watershed_properties_BCUB_with_status.geojson')
    ...
    tb = time()
    print(f'Created HYSETS data table and added attributes in {tb - ta:.2f} seconds.')

    
    print(hs_gdf.columns)
    print(hs_gdf.head())
